...
def extract_feature(image):
    ...
    ff = torch.FloatTensor(2,512).zero_().cuda()
...
def get_current_frames():
    ...
    while no_target:
        for i in range(len(process_list)):
            ...
            # save current img
            # im = Image.fromarray(boxes_img[0])
            ...
            print ('*'*10)
            print (boxes_img.shape[0])
            num_person =  boxes_img.shape[0]
            gallery_img = data_transforms(boxes_img[0])
            if num_person > 1:
                for i in range(1, num_person) :
                    gallery_img = torch.stack([gallery_img, data_transforms(boxes_img[i])], 0)
                with torch.no_grad():
            ...
            # save target feature
            # features = features[0]
            # query_feature = {'query_feature': features.numpy()}
            # scipy.io.savemat('query_result3.mat', query_feature)
            # print ('save query feature')
            # exit()
            # compute scores
            gallery_feature = features.cuda()
            query_data = scipy.io.loadmat('query_result3.mat')
            ...
            if (scores >= SCORE_THRES).any() :
                ...
                for j in range(len(process_list)) :
                    if j == i :
                        target_process = process_list[i]
                        target_queue = queue_list[i]
                        target_addr = server_list[i]
                        continue
                    process_list[j].terminate()
                break
    return target_process, target_queue, target_addr
######################################################################
# Extract feature
#
def fliplr(img):
...
tp, tq, ta = get_current_frames() # target_process, target_queue, target_addr
# do_track(tp, tq, ta)
print ('get it')
