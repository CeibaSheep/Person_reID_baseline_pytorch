...
def get_target(server_list, filename):
    no_target = True
    width = 640
    height = 480
    def worker(i,server_list,process_list):
    ...
    return max_similarity[0], max_sim_frame[0], max_sim_box[0]
...
def show_result(max_sim_frame, max_sim_box, max_similarity):
    for i in range(len(max_similarity)):
        ...
        if bbox is not None:
            ...
            ax.add_patch(r)
        image = plt.imread(max_sim_frame[i]) 
        ax.imshow(image)
    # plt.ion()
    plt.show()
    # plt.pause(0.001)
...
# print ('get it')
def start_track_single_edge(server_list,filename):
    global process_list
    global buffer_process_list
    global queue_list
    target_similarity, target_video, target_box = get_target(server_list, filename) 
    if target_box is None:
        print ("can not find this person!")
    else:
        dotracker.main(process_list[0], queue_list[0], target_box, target_video)
